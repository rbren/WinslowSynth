...
func GetADSR(name string) ADSR {
	...
	releaseMs := 1000
	return ADSR{
		PeakLevel:    Constant{name + " Level", 1.0, 0.0, 1.0},
		SustainLevel: Constant{name + " Sustain", 0.8, 0.0, 1.0},
	...
	return Multiply{
		Generators: []Generator{
			...
			Spinner{
				...
				Amplitude: Constant{name + " LFO strength", 0.0, 0.0, 100.0},
				Frequency: Constant{name + " LFO frequency", 2.0, 0.0, 20.0},
			},
		},
	}
}
...
func AddNoise(name string, base Generator) Instrument {
	return NoiseFilter{
		Input:  base,
		Amount: Constant{name + " Noise", 0.0, 0.0, 1.0},
	}
}
